...
`lfg` is the npm entry and installer for the **GrokBuild port of OpenCode OmO / lazycodex**. The project goal is to analyze the upstream OpenCode OmO implementation from `https://github.com/code-yeongyu/oh-my-openagent` and port the relevant OmO surface to GrokBuild as an lfg-owned plugin payload under native `~/.grok/plugins/lfg` (with legacy `~/.grok/installed-plugins/lfg` treated only as migration/fallback).
...
├── docs/             # adapter ownership/parity/config/publish docs; tested contracts
...
| Port OpenCode OmO behavior to GrokBuild | upstream `oh-my-openagent` + `src/grok-adapter/` + `docs/grok-adapter-parity.md` | Analyze upstream first; port as Grok-native lfg-owned behavior, not as a blind copy. |
...
| `runInternalGrokInstall()` | `src/grok-adapter/run-internal.ts` | Materializes the Grok plugin payload. |
...
- Treat `lfg` as the GrokBuild port surface for OpenCode OmO: every substantive feature change should identify the upstream OpenCode OmO behavior, decide the GrokBuild-native equivalent, and record any intentional divergence.
...
- Keep `lfgIsPlugin: false` in JSON contracts; the npm package is not reported as a Grok plugin object, even though `setup --run` installs the lfg-owned Grok plugin payload under `~/.grok/plugins/lfg`.
...
- Describing `lfg` as LFP, lazycodex itself, or a generic Grok runtime. It is the lfg-owned GrokBuild port/plugin payload for OpenCode OmO, installed by the npm CLI.
- Porting OpenCode OmO behavior without first grounding the upstream OpenCode implementation and the current GrokBuild install surface.
