...
```
bin/lfg.js (sh shim)
  └─> dist/lfg.js (src/cli/lfg.ts — routing/contract)
        └─> src/cli/lfg-installer.ts (runLazycodexInstaller)
              └─> src/grok-adapter/run-grok-install.ts (runGrokInstall — single transaction)
...
| `src/grok-adapter/` | Internal Grok installer engine: payload materialization, hook normalization, agent TOML sync, codegraph/rules ports, config single-writer. High-risk install hotspot — has its own `AGENTS.md`. |
...
| `tests/` | Narrow repo-level test scope (most tests actually live under `src/cli/` and `src/grok-adapter/`). Has its own `AGENTS.md`. |
...
- **Grok-first framing** everywhere. Default setup path is `~/.grok`; do NOT route through `npx lazycodex-ai install` into `~/.codex` (codex is optional/legacy). Grok-first model defaults; GPT/Gemini/Claude shown only as alternatives.
- **Production home resolution:** `resolveGrokSetupHome()` (`src/grok-adapter/grok-home.ts`) targets `os.userInfo().homedir`. Only tests may redirect, gated by `LFG_ALLOW_TEST_GROK_HOME=1` (with optional `LFG_TEST_GROK_HOME` explicit override).
...
- **Config single-writer:** `writeGrokModelConfig()` (`lfg-grok-config.ts`) is the only writer to `~/.grok/config.toml`'s lfg-owned sections. Endpoints hygiene: set only `endpoints.models_base_url`, remove legacy `endpoints.api_key`, put credentials under `[model.*]`. Grok OIDC auth is host-owned — lfg does NOT write `~/.grok/auth.json`.
- **Agent overrides priority:** user > discovered config > bundled Grok-first `omo-agent-overrides.json`. `lazycodex-agent-overrides.json` is the user-visible surface.
...
- `src/cli/lfg.ts` — process entry: `main()` → `parseArgs()` → `dispatch()` → `emit()`.
...
- `setup-plan.ts` — non-mutating plan builders (`setupPlan()`, `refreshPlan()`).
- `setup-json-contract.ts` — deprecated-key guards.
- `lfg-models.ts` — `fetchModelDiscovery()` (GET `/v1/models`), `applyModelPreset()` (`grok`/`gpt`), `defaultLazycodexAgentConfig()`.
- `lfg-grok-config.ts` (+ `-toml.ts`, `-sections.ts`) — config.toml single-writer; `LFG_OWNED_GROK_CONFIG_SECTIONS`.
- `lfg-setup-tui.ts` (+ `-selectors.ts`, `-data.ts`, `-agents.ts`) — `@clack/prompts` TUI (`shouldUseSetupTui()`, `runSetupTui()`).
- `lfg-interactive.ts` — `runInstallWizard()` (readline fallback).
- `npm-publish-*.ts`, `publish-readiness.ts`, `lfg-package-layout.ts` — pack/publish gating helpers.

### Install engine (`src/grok-adapter/`)
- `run-grok-install.ts` — `runGrokInstall()` single transaction.
- `run-internal.ts` — `runInternalGrokInstall()` payload materialization; source resolution priority: OMO native → lazycodex bundle → `LFG_GROK_INSTALL_SOURCE_ROOT` override → fixture-minimal fallback (with `warning`).
- `install.ts` — `installGrokPluginFromSource()` plugin path helpers.
- `sync-lazycodex-agents-to-grok.ts` — `syncLazycodexAgentsToGrokLedger()`; `GROK_AGENT_NAMES` table; `moveConflictingUserAgentsAside()`.
- `normalize-plugin-hooks.ts` — `normalizePluginHooksJson()` idempotent Grok event-map writer.
- `grok-plugins-enable.ts` — `ensureLfgPluginsEnabled/AgentsPreferred/SubagentModels`, `upsertSubagentModels()`.
- `materialize-grok-mcp.ts` — `materializeGrokMcpRuntimes()`; `resolveMcpTarget()` (runtime_packages vs components mode).
- `grok-home.ts` — `resolveGrokSetupHome()` (production vs test-home gate).
- `codegraph-manifest.ts` / `codegraph-provision.ts` / `codegraph-resolve.ts` / `codegraph-session-start.ts` / `codegraph-node-support.ts` — OpenCode OmO codegraph port (sha256-verified download to `~/.omo/codegraph`).
...
- `fixture-minimal/` — seed plugin payload used by tests and the fallback install path.
...
- `scripts/build.mjs` — esbuild bundle of 8 entry points (only `lfg-setup-tui.ts` externalizes `@clack/prompts` + `picocolors`), `dist/.build.lock` with 120s timeout, atomic `rename` staging of `fixture-minimal`, generates `mcp-runtimes/*` stubs, chmod 0755 outputs.
...
**Framework:** Vitest (`^4.0.14`, ESM). Tests live only under `src/cli/` and `src/grok-adapter/` (~110 files). `npm test` globs exactly those two directories.
...
**Testing patterns:**
- **Spawned CLI:** `src/cli/test-process.ts` exports `runLfg(args, env)` → `{exitCode, json}`; auto-builds `dist/` once per process if missing.
- **Temp homes:** `mkdtemp(join(tmpdir(), "lfg-...-"))` + `{ HOME: home }` env, relying on the global gate.
- **File fixtures:** `src/grok-adapter/fixture-minimal/` copied into temp plugin roots.
