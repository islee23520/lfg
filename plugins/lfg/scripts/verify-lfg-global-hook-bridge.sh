...
test -x "$GROK_BIN"
plugins/lfg/bin/grok-install-smoke.sh >/tmp/grok-global-bridge-install.log
BACKUP="$(mktemp -d)"
...
trap cleanup EXIT
rm -rf "$HOME/.grok/hooks" "$PWD/.lfg/events"
scripts/install-lfg-global-hook-bridge.sh >/tmp/grok-global-bridge-install.out
