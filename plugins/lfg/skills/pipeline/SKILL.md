---
name: pipeline
description: "LFG port of OMX `pipeline`: Configurable pipeline orchestrator for sequencing stages"
user_invocable: true
metadata:
...
# Pipeline Skill — LFG Port

This skill is the LFG adaptation of the OMX `pipeline` workflow from `oh-my-codex`.
...
5. Store durable workflow notes under `.lfg/pipeline/` when state is needed.
