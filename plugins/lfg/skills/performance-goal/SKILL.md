...
description: "LFG port of OMX `performance-goal`: Run an evaluator-gated performance optimization workflow over Codex goal mode with durable OMX artifacts and safe goal handoffs."
user_invocable: true
metadata:
...
# Performance Goal Workflow — LFG Port

This skill is the LFG adaptation of the OMX `performance-goal` workflow from `oh-my-codex`.
...
5. Store durable workflow notes under `.lfg/performance-goal/` when state is needed.
