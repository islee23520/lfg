---
name: ask
description: "LFG port of OMX `ask`: Ask a local external advisor CLI (Claude or Gemini) and capture a reusable artifact"
user_invocable: true
metadata:
...
# Ask (Local Advisor CLI) — LFG Port

This skill is the LFG adaptation of the OMX `ask` workflow from `oh-my-codex`.
...
5. Store durable workflow notes under `.lfg/ask/` when state is needed.
