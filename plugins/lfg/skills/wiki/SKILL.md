---
name: wiki
description: "LFG port of OMX `wiki`: Persistent markdown project wiki stored under repository omx_wiki with keyword search and lifecycle capture"
user_invocable: true
metadata:
...
This skill is the LFG adaptation of the OMX `wiki` workflow from `oh-my-codex`.
...
5. Store durable workflow notes under `.lfg/wiki/` when state is needed.
