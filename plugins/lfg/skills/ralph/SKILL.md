---
name: ralph
description: "LFG port of OMX `ralph`: Self-referential loop until task completion with architect verification"
user_invocable: true
metadata:
...
This skill is the LFG adaptation of the OMX `ralph` workflow from `oh-my-codex`.
...
5. Store durable workflow notes under `.lfg/ralph/` when state is needed.
