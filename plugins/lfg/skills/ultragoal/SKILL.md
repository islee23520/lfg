---
name: ultragoal
description: "LFG port of OMX `ultragoal`: Create and execute durable repo-native multi-goal plans over Codex goal mode artifacts."
user_invocable: true
metadata:
...
# Ultragoal Workflow — LFG Port

This skill is the LFG adaptation of the OMX `ultragoal` workflow from `oh-my-codex`.
...
5. Store durable workflow notes under `.lfg/ultragoal/` when state is needed.
...
1. **Restate + Checklist (this session)**
   ...
   - Build a concise scenario/checklist in a durable note under `.lfg/ultragoal/<id>/plan.md` (or append to ledger).
...
2. **Create the Ultragoal Plan**
   ...
   lfg ultragoal create "Ship full OMX parity for lfg" --id my-ug-1 \
     --brief "Make lfg match oh-my-codex ultragoal experience end-to-end." \
     --checklist "design;implement;gate;verify"
   ```
   This produces:
   - `.lfg/ultragoal/my-ug-1/brief.md`
...
This gives LFG the same "durable repo-native multi-goal plans over goal artifacts" power that OMX gives Codex, using only Grok-native paths (`lfg`/`lfg.py`, MCP, `.lfg/`).
