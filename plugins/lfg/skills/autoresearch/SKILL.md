---
name: autoresearch
description: "LFG port of OMX `autoresearch`: Stateful validator-gated research loop with native-hook persistence"
user_invocable: true
metadata:
...
# Autoresearch — LFG Port

This skill is the LFG adaptation of the OMX `autoresearch` workflow from `oh-my-codex`.
...
5. Store durable workflow notes under `.lfg/autoresearch/` when state is needed.
