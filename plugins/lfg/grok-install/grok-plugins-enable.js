...
async function ensureLfgPluginsEnabled(home = homedir()) {
...
async function ensureLfgAgentsPreferred(home = homedir()) {
  const path = join(home, ".grok", "config.toml");
  const current = await readTextIfExists(path);
...
async function ensureLfgSubagentModels(home = homedir(), mapping = {}) {
  const path = join(home, ".grok", "config.toml");
  const current = await readTextIfExists(path);
...
function upsertSubagentModels(source, mapping = {}) {
  ...
  const block = Object.entries(lfgOwned).map(([key, model]) => `${key} = ${tomlString(model)}`).join("\n");
...
export {
  ensureLfgAgentsPreferred,
  ensureLfgPluginsEnabled,
  ensureLfgSubagentModels,
  upsertSubagentModels
};
