...
```sh
npm run build
vitest run plugins/lfg/grok-install/*.test.ts
vitest run plugins/lfg/bin/lfg-grok-install.test.ts
node plugins/lfg/dist/lfg.js --json setup --run
node plugins/lfg/dist/lfg.js --json setup --run --force
```
