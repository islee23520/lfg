...
export async function runInternalGrokInstall(env = process.env) {
    const home = env.HOME ?? homedir();
    const version = env.LFG_PACKAGE_VERSION ??
    ...
    const canRepairCleanly = !forceReinstall &&
        resolved !== null &&
    ...
    const lazycodexSource = sourceOverride && sourceOverride.length > 0
        ? sourceOverride
...
async function finishRepair(pluginRoot, pluginDirName, version, mode) {
