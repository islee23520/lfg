...
const PROJECT_OMO_LEDGER_FILE = "lfg-project-omo-ledger.mjs";
...
export function normalizeHookCommandPaths(command) {
    return command.replace(PLUGIN_ROOT_PLACEHOLDER, "${GROK_PLUGIN_ROOT}");
}
export async function syncGrokHookBridgeIntoPlugin(pluginRoot) {
...
export async function normalizePluginHooksJson(pluginRoot) {
    ...
    for (const [eventName, groups] of Object.entries(hooksBlock)) {
        ...
        nextBlock[eventName] = groups.map((group) => normalizeHookGroup(group, () => {
            changed = true;
        }));
    }
 ...
 */
export function wrapLazyCodexHookCommand(command) {
