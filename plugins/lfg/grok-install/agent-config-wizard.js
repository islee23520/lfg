...
export async function configureOmoAgentOverridesInteractively(reader, discovery, roleConfig, writeLine, confirm) {
...
async function readModelChoice(reader, discovery, writeLine, prompt, fallback) {
    writeLine(prompt);
...
async function readReasoningLevel(reader, writeLine, prompt, fallback) {
    writeLine(prompt);
    const answer = await reader.next();
