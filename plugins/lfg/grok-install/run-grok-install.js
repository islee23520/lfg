...
export async function runGrokInstall(discovery, env = process.env, options = {}) {
    ...
    if (existingSetup !== null) {
        ...
        const overrideMap = agentOverrideMap !== undefined
            ? agentOverrideMap
            : applyRecommendationsToOverrideMap(await resolveLazycodexAgentOverrides(home, resolvedAgents), discovery?.modelIds ?? [], discovery?.preset);
        ...
        const subagentModels = await ensureLfgSubagentModels(home, subagentModelMapping(resolvedAgents.explorer?.model ?? resolvedAgents.reasoning?.model, resolvedAgents.reasoning?.model, resolvedAgents.coding?.model));
    ...
    const pluginsEnabled = await ensureLfgPluginsEnabled(home);
    await ensureLfgAgentsPreferred(home);
    const subagentModels = await ensureLfgSubagentModels(home, subagentModelMapping(resolvedAgents.explorer?.model ?? "grok-3-mini-fast", resolvedAgents.reasoning?.model ?? "grok-4.20-0309-reasoning", resolvedAgents.coding?.model ?? "grok-4.20-0309-non-reasoning"));
