...
import { CONFIGURABLE_LAZYCODEX_AGENT_NAMES, loadBundledDefaultOmoOverrides, mergeLazycodexAgentOverrides, } from "../grok-install/lazycodex-agent-overrides";
import { INTERNAL_GROK_INSTALL_COMMAND } from "../grok-install/run-grok-install";
export function shouldUseSetupTui(args, options) {
    if (options.check || args.noTui === true)
        return false;
    return options.input?.isTTY === true && options.output?.isTTY === true;
}
const ROLE_AGENT_NAMES = ["explorer", "reasoning", "coding"];
const ROLE_AGENT_NAME_SET = new Set(ROLE_AGENT_NAMES);
...
export async function runSetupTui(args, context, deps = {}) {
    ...
    try {
        ...
        const installRes = await runLazycodexInstaller(configuredForInstall);
...
function readDiscoveryFromContext(context) {
    ...
    if (isRecord(resolved) && isModelDiscovery(resolved.discovery))
        return resolved.discovery;
    const plan = context.plan;
    if (isRecord(plan) && isModelDiscovery(plan.modelDiscovery))
        return plan.modelDiscovery;
    return null;
}
...
async function configureAgent(name, currentModel, currentReasoning, choices, selectors) {
...
async function configureAgentOverrides(prompts, discovery, choices, selectors, roleConfig, bundled) {
