...
for rel in ["skills/lfg/SKILL.md", "agents/harness.toml", "hooks/scripts/lfg-audit-hook.sh", "bin/lfg-mcp.py", "bin/grok-install-smoke.sh"]:
...
for required in ["runtime-smoke-coverage=100%", "release-readiness-local=ok", "scripts/verify-release-readiness-local.sh", "scripts/verify-release-readiness-remote.sh", "scripts/verify-release-readiness-all.sh", "scripts/install-lfg-symlink.sh", "scripts/verify-lfg-launch.sh", "scripts/verify-team-tmux-lifecycle.sh", "lfg-status=ok version=0.3.0", "lfg-doctor=ok", "team-preflight-cli=ok", "team-preflight-commands=ok", "team-preflight-slash=ok", "team-preflight-mcp=ok", "team-provider-matrix=ok", "team-provider-slash=ok", "team-provider-commands=ok", "team-tmux-lifecycle=ok", "mcp-stdio-isolation=ok", "state-schema-versioning=ok", "release-notes=ok", "marketplace-source=ok", "grok-hook-discovery=ok", "hook-event-replay=ok", "grok-headless-session=ok", "grok-global-hook-bridge=ok", "grok-installed-mcp-surface=ok", "grok_build_team.preflight", "commands=ok", "lfg-installed-symlink-surface=ok", "/team providers", "/team preflight", "lfg-inside-tmux-attach=ok", "release-tag=ok", "grok-plugins-surface=ok", "grok-install-smoke=ok skills=28 key_skills_present", "remote-smoke=ok", "roadmap=27/27", "feature_docs=27/27", "linalab-io/lfg", "grok_marketplace", "agents_marketplace"]:
