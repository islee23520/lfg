...
for rel in ["skills/lfg/SKILL.md", "src/agents/harness.toml", "hooks/scripts/lfg-audit-hook.sh", "bin/lfg-mcp.py", "bin/grok-install-smoke.sh"]:
...
assert "lfg-runtime" in launch_lfg
...
bridge = (repo / "scripts/verify-lfg-global-hook-bridge.sh").read_text()
assert "grok-global-hook-bridge=ok" in bridge
assert (repo / "scripts/install-lfg-global-hook-bridge.sh").stat().st_mode & 0o111, "scripts/install-lfg-global-hook-bridge.sh executable"
assert (repo / "scripts/verify-lfg-global-hook-bridge.sh").stat().st_mode & 0o111, "scripts/verify-lfg-global-hook-bridge.sh executable"
...
all_ready = (repo / "scripts/verify-release-readiness-all.sh").read_text()
assert "release-readiness-all=ok" in all_ready
assert "verify-release-readiness-local.sh" in all_ready
assert "verify-release-readiness-remote.sh" in all_ready
assert (repo / "scripts/verify-release-readiness-all.sh").stat().st_mode & 0o111, "scripts/verify-release-readiness-all.sh executable"
remote_ready = (repo / "scripts/verify-release-readiness-remote.sh").read_text()
assert "release-readiness-remote=ok" in remote_ready
assert "verify-remote-smoke.sh" in remote_ready
assert "verify-release-tag.sh" in remote_ready
assert (repo / "scripts/verify-release-readiness-remote.sh").stat().st_mode & 0o111, "scripts/verify-release-readiness-remote.sh executable"
release_ready = (repo / "scripts/verify-release-readiness-local.sh").read_text()
assert "release-readiness-local=ok" in release_ready
assert "verify-installed-lfg-symlink-surface.sh" in release_ready
assert "verify-grok-installed-mcp-surface.sh" in release_ready
assert (repo / "scripts/verify-release-readiness-local.sh").stat().st_mode & 0o111, "scripts/verify-release-readiness-local.sh executable"
smoke_doc = (repo / "docs/SMOKE.md").read_text()
for required in ["plugins/lfg/bin/self-test.sh", "scripts/verify-release-readiness-local.sh", "scripts/verify-release-readiness-remote.sh", "scripts/verify-release-readiness-all.sh", "scripts/install-lfg-symlink.sh", "scripts/verify-lfg-launch.sh", "scripts/verify-team-preflight.sh", "scripts/verify-team-provider-commands.sh", "scripts/verify-team-tmux-lifecycle.sh", "scripts/verify-mcp-stdio-isolation.sh", "scripts/verify-state-schema.sh", "scripts/verify-release-notes.sh", "scripts/verify-marketplace-source.sh", "scripts/verify-grok-hook-discovery.sh", "scripts/verify-lfg-global-hook-bridge.sh", "scripts/verify-grok-installed-mcp-surface.sh", "scripts/verify-installed-lfg-symlink-surface.sh", "scripts/verify-lfg-inside-tmux-attach.sh", "scripts/verify-release-tag.sh", "scripts/verify-grok-plugins-surface.sh", "plugins/lfg/bin/grok-install-smoke.sh", ".github/workflows/smoke.yml", "scripts/verify-remote-smoke.sh p1", "runtime-smoke-coverage=100%", "lfg-status=ok version=0.3.0", "lfg-doctor=ok", "lfg-launch-smoke=ok", "team-preflight-cli=ok", "team-preflight-commands=ok", "team-preflight-slash=ok", "team-preflight-mcp=ok", "team-provider-matrix=ok", "team-provider-slash=ok", "team-provider-commands=ok", "team-tmux-lifecycle=ok", "mcp-stdio-isolation=ok", "state-schema-versioning=ok", "release-notes=ok", "marketplace-source=ok", "grok-hook-discovery=ok", "hook-event-replay=ok", "grok-headless-session=ok", "grok-global-hook-bridge=ok", "grok-installed-mcp-surface=ok", "grok_build_team(action=preflight)", "commands=ok", "lfg-installed-symlink-surface=ok", "/team providers", "/team preflight", "lfg-inside-tmux-status=ok", "release-tag=ok", "grok-plugins-surface=ok", "grok-install-smoke=ok skills=28", "remote-smoke=ok"]:
...
for required in ["runtime-smoke-coverage=100%", "release-readiness-local=ok", "scripts/verify-release-readiness-local.sh", "scripts/verify-release-readiness-remote.sh", "scripts/verify-release-readiness-all.sh", "scripts/install-lfg-symlink.sh", "scripts/verify-lfg-launch.sh", "scripts/verify-team-tmux-lifecycle.sh", "lfg-status=ok version=0.3.0", "lfg-doctor=ok", "team-preflight-cli=ok", "team-preflight-commands=ok", "team-preflight-slash=ok", "team-preflight-mcp=ok", "team-provider-matrix=ok", "team-provider-slash=ok", "team-provider-commands=ok", "team-tmux-lifecycle=ok", "mcp-stdio-isolation=ok", "state-schema-versioning=ok", "release-notes=ok", "marketplace-source=ok", "grok-hook-discovery=ok", "hook-event-replay=ok", "grok-headless-session=ok", "grok-global-hook-bridge=ok", "grok-installed-mcp-surface=ok", "grok_build_team.preflight", "commands=ok", "lfg-installed-symlink-surface=ok", "/team providers", "/team preflight", "lfg-inside-tmux-status=ok", "release-tag=ok", "grok-plugins-surface=ok", "grok-install-smoke=ok skills=28 key_skills_present", "remote-smoke=ok", "roadmap=27/27", "feature_docs=27/27", "linalab-io/lfg", "grok_marketplace", "agents_marketplace"]:
