...
const PUBLIC_LITELLM_CATALOG_URL = "https://raw.githubusercontent.com/BerriAI/litellm/main/model_prices_and_context_window.json";
export async function loadPublicLiteLLMContextMap() {
    ...
    try {
        ...
        for (const [rawKey, spec] of Object.entries(data)) {
            if (rawKey === "sample_spec" || !isRecord(spec))
                continue;
            ...
            if (!out[normalized])
                out[normalized] = contextWindow;
            const providerStripped = rawKey.includes("/") ? rawKey.split("/").pop() : rawKey;
            if (providerStripped === undefined)
                continue;
            const strippedNormalized = aliasGroupKey(providerStripped);
            if (!out[strippedNormalized])
                out[strippedNormalized] = contextWindow;
        }
        return out;
    }
    catch {
...
export function aliasGroupKey(modelId) {
    return modelId
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-+|-+$/g, "");
