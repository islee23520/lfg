...
describe("setup-json-contract (#21)", () => {
    test("successful setup --run has no deprecated adapter repair keys", async () => {
        ...
        const verify = json.postInstallVerify;
    ...
    test("findDeprecatedSetupJsonKeys detects legacy fields", () => {
        ...
        expect(findDeprecatedSetupJsonKeys({ mcpConfigRepair: {}, stablePluginLinks: [] })).toEqual(expect.arrayContaining(["mcpConfigRepair", "stablePluginLinks"]));
