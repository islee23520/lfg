...
export async function writeGrokModelConfig(discovery, options = {}) {
    ...
    const next = upsertSection(withAgents, "lazycodex.models", [
...
export function grokConfigJson(update) {
    return {
 ...
 */
export async function refreshGrokModelConfig(discovery, options = {}) {
    const home = options.home ?? homedir();
    if (discovery === null) {
        return {
            ok: false,
...
function upsertLazycodexAgentSections(source, agentConfig) {
    return Object.entries(agentConfig).reduce((next, [agentName, setting]) => upsertSection(next, `lazycodex.agents.${agentName}`, [
...
function upsertAllLazycodexAgentSections(source, full) {
    return Object.entries(full).reduce((next, [agentName, setting]) => isBareKey(agentName)
