...
describe("lfg internal grok install contract", () => {
    ...
    test("setup --run surfaces payloadSource and component inventory in postInstallVerify (supports #38/#42)", async () => {
        ...
        const result = await runLfg(["--json", "setup", "--run"], { HOME: home });
        expect(result.exitCode).toBe(0);
        const p = result.json.postInstallVerify;
        expect(p).toBeTruthy();
        ...
        // T4 failing-first for doctor/post-install reporting (native hook, bridge fallback, OMO components, skill workflows). JSON stable/secret-safe. Will fail until T9.
    ...
    test("setup --run with fixture fallback includes warning and payloadSource=fixture_fallback in internalStep (supports #38)", async () => {
        ...
        expect(result.exitCode).toBe(0);
        const internal = result.json.internalStep;
    ...
    test("setup --run installs executable rules and ultrawork hook bridge targets", async () => {
        ...
        const ultrawork = await runInstalledHook([bridgePath, "node", join(pluginRoot, "components", "ultrawork", "dist", "cli.js"), "hook", "user-prompt-submit"], { hookEventName: "user_prompt_submit", sessionId: "test-session", cwd: process.cwd(), prompt: "work" }, { GROK_PLUGIN_ROOT: pluginRoot, GROK_HOOK_EVENT: "user_prompt_submit" });
...
function runInstalledHook(argv, payload, env) {
