---
name: worker
description: "Grok Build port of OMX `worker`: Team worker protocol (ACK, mailbox, task lifecycle) for tmux-based OMX teams"
user_invocable: true
metadata:
...
# Worker Skill — Grok Build Port

This skill is the Grok Build adaptation of the OMX `worker` workflow from `oh-my-codex`.
...
- Source skill: `oh-my-codex/plugins/oh-my-codex/skills/worker/SKILL.md`
- Original description: Team worker protocol (ACK, mailbox, task lifecycle) for tmux-based OMX teams
...
5. Store durable workflow notes under `~/.grok/plugin-data/grok-build/worker/` when state is needed.
