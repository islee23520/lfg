// 파쿠리'd from senpi bash-timeout/timeout.ts (policy-text half only). Grok hooks can
// inject context but cannot rewrite a tool's `timeout` arg, so senpi's input-mutation
// half is a host-surface gap intentionally omitted here.
...
export function resolveBashTimeoutDefaults(env: EnvLike): BashTimeoutDefaults {
  const defaultSeconds =
    parsePositiveInt(env["LFG_BASH_DEFAULT_TIMEOUT_SECONDS"]) ?? BASH_DEFAULT_TIMEOUT_SECONDS
  const rawMax =
    parsePositiveInt(env["LFG_BASH_MAX_TIMEOUT_SECONDS"]) ?? BASH_MAX_TIMEOUT_SECONDS
