...
describe("resolveGrokHookBridgeAssetPath", () => {
  test("finds bridge from grok-adapter module (source layout)", async () => {
    const path = await resolveGrokHookBridgeAssetPath(import.meta.url)
    expect(path).toContain("grok-adapter/assets/lfg-grok-hook-bridge.mjs")
  })
  ...
  test("finds bridge from dist/lfg.js bundle layout", async () => {
    ...
    expect(path).toContain("dist/grok-install/assets/lfg-grok-hook-bridge.mjs")
  })
})