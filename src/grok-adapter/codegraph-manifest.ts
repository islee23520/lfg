import type { CodegraphProvisionManifest } from "./codegraph-provision"

/**
 * Codegraph provision manifest (sha256-verified download map).
 *
 * Ported verbatim from `oh-my-openagent` `packages/utils/src/codegraph/manifest.ts`
 * so lfg can provision the external `@colbymchenry/codegraph` binary the same way
