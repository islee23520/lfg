...
describe("lazycodex-agent-overrides", () => {
  ...
  test("bundled defaults include default, ulw, sisyphus, and atlas agents", async () => {
    ...
    expect(bundled.sisyphus?.model).toBe("glm-5.2")
  ...
  test("bundled defaults include oracle and sisyphus-junior OMO parity agents", async () => {
    const bundled = await loadBundledDefaultOmoOverrides()
    expect(bundled.oracle?.model).toBe("grok-4.20-0309-reasoning")
    expect(bundled.oracle?.reasoningLevel).toBe("high")
    expect(bundled.oracle?.modelFallback).toBe("gemini-3.1-pro-low")
    expect(bundled["sisyphus-junior"]?.model).toBe("grok-4.20-0309-reasoning")
  ...
  test("bundled defaults include OMO category agents (ultrabrain, deep, quick, unspecified-low, unspecified-high, writing)", async () => {
    const bundled = await loadBundledDefaultOmoOverrides()
    expect(bundled.ultrabrain?.model).toBe("grok-4.3")
    expect(bundled.ultrabrain?.reasoningLevel).toBe("xhigh")
    expect(bundled.deep?.model).toBe("grok-4.20-0309-reasoning")
    ...
    expect(bundled["unspecified-high"]?.model).toBe("grok-4.20-0309-reasoning")
