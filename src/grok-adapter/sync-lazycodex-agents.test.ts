...
describe("sync lazycodex agents to grok", () => {
  test("runGrokInstall writes grok-compatible explorer from fixture ultrawork tree", async () => {
    ...
    expect(defaultAgent).toContain("Grok-native OMO Hephaestus")
    const defaultPrompt = await readFile(join(home, ".grok", "prompts", "lazycodex", "default.md"), "utf8")
    expect(defaultPrompt).toContain("Grok-native OMO Hephaestus")
