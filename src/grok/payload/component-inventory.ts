...
export const COMPONENTS = [
  ...
  { id: "teammode", status: "Deferred", evidence: "Upstream 4.13.0 teammode skill payload installed + Grok MVP ledger in src/core/lfg/team-ledger.ts (createTeam/addMemberSlot/recordSpawnMetadata/appendMessage/list/getStatus/requestShutdown; durable .omo/teams JSON with spawn_subagent-shaped mapping). T6 unit e2e GREEN (4 tests: full lifecycle + fail-closed + skill API; .omo/evidence/omo-parity-gap-fill-88-to-95/T6-tests.txt). T7 HOLD Deferred: live multi-spawn_subagent host e2e and setup receipt not proven — no Grok-adapted claim (host dependency class: codex_app; follow-up #100; see T7-promote-or-hold.md, grok-native-team-orchestration.md)." },
  { id: "lazycodex-executor-verify", status: "Deferred", evidence: "Upstream 4.13.0 SubagentStop evidence verifier targets the Codex lazycodex-executor agent (host dependency class: Stop/SubagentStop hook). MVP pure function src/grok/hooks/subagent-stop-evidence-verifier.ts + T3: same verifySubagentStopEvidence logic wired into lfg-sisyphus-hooks.mjs SubagentStop additionalContext for coding|hephaestus|builder (.omo/evidence receipt or WARNING, fail-closed malformed JSON; e2e subagent-stop-verifier-e2e.test.ts). Residual: no dedicated lazycodex-executor-verify component CLI / host-enforced block-or-continue; remains Deferred until full host-enforced verifier proven." },
  { id: "workflow-selector", status: "Deferred", evidence: "Upstream 4.13.0 workflow-selector is an opt-in Codex UserPromptSubmit hook that emits hookSpecificOutput.additionalContext. lfg does not enable it until a Grok-native prompt-routing hook surface is implemented and verified (host dependency class: missing host surface). T9 residual WAIVE (issue #102): no fake Grok-adapted workflow selector; remains Deferred. See .omo/evidence/omo-parity-gap-fill-88-to-95/T9-residuals.txt." },
