...
export const COMPONENTS = [
  ...
  { id: "start-work-continuation", status: "Deferred", evidence: "Boulder/start-work continuation is not yet driven as a Grok-native lifecycle workflow; the durable continuation CLI is not packaged, while Sisyphus Stop/SubagentStop hooks provide context only." },
  { id: "prompts-core", status: "Grok-adapted", evidence: "Phase 3 of the core/adapter port strategy: prompts-core source (types, loader, variant-resolver, prompt tables for atlas/prometheus/ultrawork/mode) vendored under src/grok/ports/vendor/prompts-core-vendored/; Grok glue in ports/grok-prompt-adapter.ts resolves the default variant for Grok models via the fallback chain (docs/grok-adapter-core-port-strategy.md)." },
  { id: "agent-builder", status: "Grok-adapted", evidence: "Phase 4 of the core/adapter port strategy: agent-builder foundation + dynamic-agent prompt builders + curated builtin agent registry vendored under src/grok/ports/vendor/agent-builder-vendored/; Grok glue in ports/grok-agent-builder-adapter.ts assembles Grok agent roles (5/9 agents fully ported; oracle/metis/sisyphus/hephaestus deferred as host-bound). See docs/grok-adapter-core-port-strategy.md." },
