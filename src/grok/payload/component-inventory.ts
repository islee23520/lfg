...
export const COMPONENTS = [
  ...
  { id: "difficulty-tier-workers", status: "Grok-adapted", evidence: "Host-neutral resolveDifficultyTierRoute (src/core/lfg/subagents/difficulty-tier-routing.ts) + omo-spawn-map identity for lazycodex-worker-low|medium|high; Grok install materializes roles/agents/prompts + [subagents.models]/[omo.agents] model/effort routes (subagent-routing.ts); fresh/preserve proof in run-grok-install-default-agents.test.ts. Orchestrator-selected spawn_subagent routing/guidance only — NOT host-enforced automatic classification, OpenCode transforms, Codex agent_type, or MultiAgentV2 mailbox." },
