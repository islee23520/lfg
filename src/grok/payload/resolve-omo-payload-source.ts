...
async function isBundledOmoPayload(root: string): Promise<boolean> {
  try {
    await access(join(root, "assets", "hooks", "lfg-grok-hook-bridge.mjs")).catch(async () => access(join(root, "assets", "lfg-grok-hook-bridge.mjs")))
    await access(join(root, "assets", "config", "lfg-config-loader.mjs")).catch(async () => access(join(root, "assets", "lfg-config-loader.mjs")))
    await access(join(root, "hooks", "hooks.json"))
    await access(join(root, "fixture", "hooks", "hooks.json"))
    return true
  } catch {
    return false
  }
