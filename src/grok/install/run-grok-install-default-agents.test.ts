...
const RETIRED_DEFAULT_AGENTS = ["default", "hephaestus", "prometheus", "atlas", "oracle", "librarian", "metis", "momus", "multimodal-looker", "sisyphus-junior", "lazycodex", "lazycodex-worker-low", "lazycodex-worker-medium", "lazycodex-worker-high"] as const

describe("runGrokInstall slim default agent surfaces", () => {
  test("materializes slim natives including git-master", async () => {
    ...
    expect(configToml).toContain('default = "sisyphus"')
    expect(configToml).toMatch(/\[agent\]\s*\nname\s*=\s*"sisyphus"/)
    expect(configToml).toContain('"default"')
