...
describe("runGrokInstall", () => {
  ...
  test("existing stamped setup preserves install assets while syncing discovered config unless force is explicit", async () => {
    ...
    await expect(readFile(join(home, ".grok", "plugins", "lfg", "agents", "prometheus.md"), "utf8")).rejects.toMatchObject({ code: "ENOENT" })
    ...
    await expect(readFile(join(home, ".grok", "roles", "default.toml"), "utf8")).rejects.toMatchObject({ code: "ENOENT" })
    await expect(readFile(join(home, ".grok", "roles", "prometheus.toml"), "utf8")).rejects.toMatchObject({ code: "ENOENT" })
    await expectUpstreamOmoWorkflowSkills(join(home, ".grok", "plugins", "lfg"))
    ...
    await expect(readFile(configPath, "utf8")).resolves.not.toContain("[omo.models]")
    await expect(readFile(userUlwPath, "utf8")).rejects.toMatchObject({ code: "ENOENT" })
    await expect(readFile(ulwBackup, "utf8")).resolves.toBe(USER_GROK_AGENTS_ULW_SEED)
    await expect(readFile(join(home, ".grok", "plugins", "lfg", "agents", "prometheus.md"), "utf8")).rejects.toMatchObject({ code: "ENOENT" })
  }, 30_000)
  ...
  test("existing stamped setup writes full agent model overrides from setup choices", async () => {
    ...
    expect(config).not.toContain("[omo.agents.plan]")
