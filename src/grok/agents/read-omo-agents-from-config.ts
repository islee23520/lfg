...
import type { LazycodexAgentConfig, LazycodexAgentName, ReasoningLevel } from "../../cli/models/lfg-models"
...
export async function readOmoAgentsFromGrokConfig(home: string): Promise<LazycodexAgentConfig | null> {
  const omoResult = await readAgentsFromSection(home, "omo")
  if (omoResult !== null) {
    return omoResult
  }
  return readLazycodexAgentsFromGrokConfig(home)
}

async function readAgentsFromSection(home: string, namespace: string): Promise<LazycodexAgentConfig | null> {
  ...
  const models = readModelsSection(text, namespace)
  const partial: { [K in LazycodexAgentName]?: { model: string; reasoningLevel: ReasoningLevel } } = {}
  for (const name of AGENT_NAMES) {
    const section = readTomlSection(text, `${namespace}.agents.${name}`)
...
function readModelsSection(
  text: string,
  namespace: string,
): { readonly default: string | null; readonly reasoning: string | null; readonly coding: string | null } {
  const section = readTomlSection(text, `${namespace}.models`)
  return {
