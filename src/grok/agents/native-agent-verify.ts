...
import { BUILTIN_OVERLAY_AGENT_NAMES, NATIVE_HEPHAESTUS_MARKER, NATIVE_OMO_AGENT_NAMES, NATIVE_SISYPHUS_MARKER } from "./native-omo-agents"
...
export async function verifyNativeOmoAgents(pluginRoot: string, home: string): Promise<NativeAgentsVerifyResult> {
  ...
  const allNativeAgentsPresent =
    ...
    NATIVE_OMO_AGENT_NAMES.every((name) => prompts.includes(name) || (BUILTIN_OVERLAY_AGENT_NAMES.has(name) && builtinPrompts.includes(name)))
