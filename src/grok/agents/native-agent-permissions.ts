...
export function grokCeoMandatoryCommandsBlock(): string {
  return [
    ...
    "  4) lfg --json handoff plan --role coding --engine gpt --focus \"…\"",
    "     then launch handoff.launch → Codex (engine gpt). Do not implement yourself.",
    "",
...
export function grokCeoJudgmentPolicyBlock(): string {
  return [
    ...
    "  → hand a clear brief directly to Codex (`lfg --json handoff plan --engine gpt`; app-server first, codex exec fallback).",
    "  → watch all threads in parallel (M1–M4); wait for RESULT (STATUS / EVIDENCE / RISKS).",
    ...
    "FORBIDDEN: authoritative product judgment without a Codex query/RESULT; product file edits; fake pass.",
...
export function nativeAgentPermissionPolicyBlock(name: NativeOmoAgentName | string): string {
  ...
  if (name === "sisyphus") {
    return [
      "<lfg-agent-permissions>",
      "ROLE: Sisyphus = Grok CEO + orchestrator.",
      "YOU MAY: set goals, write assignment briefs, launch Codex/git-master handoffs, passively read status/RESULT, present synthesized decisions.",
      "FORBIDDEN: edit/write/delete product files; apply_patch; search_replace; multi_edit; mutate the repo.",
      "FORBIDDEN: final technical judgment from Grok alone — ALWAYS query Codex and integrate RESULT.",
      "IMPLEMENTATION + JUDGMENT PATH: `lfg --json handoff plan` → launch returned Codex argv. Git PATH: git-master agent.",
      "If you are about to decide or edit yourself: STOP → handoff to Codex (or git-master for pure git).",
      "</lfg-agent-permissions>",
  ...
  return [
    ...
    "IMPLEMENTATION + JUDGMENT PATH: only external Codex through `lfg --json handoff plan`.",
    "</lfg-agent-permissions>",
  ].join("\n")
}
