...
describe("native agent permissions", () => {
  ...
  test("fallback prompts embed permission + CEO judgment policy", () => {
    ...
    expect(nativeOmoFallbackPrompt("sisyphus")).toContain("goal drive --skill ulw-loop --skill programming")
    expect(nativeOmoFallbackPrompt("sisyphus")).toContain("FORBIDDEN: product file mutation")
    expect(nativeOmoFallbackPrompt("sisyphus")).not.toContain("YOU MAY implement or edit yourself")
