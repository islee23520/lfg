---
name: teammode
description: "GrokBuild + Codex team orchestration: run a named team of cooperating workers with durable script-managed state under .omo/teams. MUST USE when the user asks to create, run, coordinate, inspect, archive, or delete a team of agents/subagents/sessions, or to work as a team in parallel. On GrokBuild, FIRST select transport spawn_subagent (host spawn_subagent + bind-subagent). On Codex, select multi_agent_v2 or codex_app. The main session is always the leader; members are concrete part/ownership/perspective slices - never vague job roles. Triggers: team mode, teammode, make a team, run as a team, team of agents, parallel subagents, coordinate workers, archive the team."
---

## GrokBuild teammode (primary on lfg)

On Grok Build with lfg, teammode is **supported**. Do **not** stop because `codex_app` or MultiAgentV2 is missing. NO codex_app required on Grok.
...
3. Pick **`--subagent-type` from both catalogs** (see below) — GrokBuild built-ins **and** lfg OMO / convenience agents.
...
6. Optional durable ledger: `src/core/lfg/team-ledger.ts` under `.omo/teams` — exports `createTeam`, `addMemberSlot`, `recordSpawnMetadata`, `appendMessage`, `listTeams`, `getStatus`, `requestShutdown`. Fails closed on invalid input. teammode is Grok-adapted via spawn_subagent transport.

### Member agent catalogs (use both)

**GrokBuild host built-ins** (always available with `spawn_subagent`):

| `subagent_type` | Use for |
| --- | --- |
| `general-purpose` | Generic worker when no specialist fits |
| `explore` | Fast read-only codebase search (host built-in) |
| `plan` | Planning / design slice (host built-in; also lfg agent name) |

**lfg-installed OMO / convenience agents** (after `lfg setup --run` agent sync):
...
| `momus` / `reviewer` | Plan / code review |
| `atlas` / `sisyphus` / `default` | Orchestration-shaped slices (rare as members) |
| `multimodal-looker` | Vision / screenshot / PDF |

Aliases accepted by `team.mjs`: `search`→`explore`, `deep`/`worker`→`hephaestus`, `impl`→`coding`, `qa`/`builder`→`reviewer`, `research`→`librarian`. Prefer a **specialist** (lfg or built-in) over `general-purpose` when the slice is clear.
...
# after each spawn_subagent returns an id:
...
This workflow uses a bundled state script under `.omo/teams`. On GrokBuild the transport is
`spawn_subagent`. On Codex, choose native MultiAgentV2 agents or Codex App threads. Never mix
transports on one team.
...
3. **GrokBuild / lfg** - select `spawn_subagent` when host `spawn_subagent` is available (this is the lfg path).
4. **Neither Codex set nor spawn_subagent** - STOP before `init`. Tell the user which tools are
...
Pass that choice to `init` as `--transport spawn_subagent`, `multi_agent_v2`, or `codex_app`. The
...
```
node "<skill-root>/scripts/team.mjs" init        --name "<team>" --session-name "<session>" --transport spawn_subagent|multi_agent_v2|codex_app [--session <leader_thread_id>] [--worktree] [--base-branch dev]
