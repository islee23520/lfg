...
**Per `docs/grok-native-team-orchestration.md` (design contract for checkbox 5).** Durable state lives under project `.omo/teams/{teamRunId}/` (project override wins; falls back to `~/.omo/teams`). The `src/core/lfg/team-ledger.ts` module provides:
...
- `addMemberSlot(teamsRoot, teamId, spec)` — adds member slot (supports subagent_type or category)
- `recordSpawnMetadata(teamsRoot, teamId, memberId, metadata)` — records `spawn_subagent` result (subagentId, etc.); **fails closed** on missing memberId.
- `appendMessage(teamsRoot, teamId, from, to, content)` — durable mailbox
