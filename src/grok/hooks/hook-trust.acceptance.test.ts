...
describe("native grok hook json contract (T1)", () => {
  test("fixture hooks.json validates from disk as native Grok event map", async () => {
    const hooksPath = join(dirname(fileURLToPath(import.meta.url)), "..", "fixture", "hooks", "hooks.json")
  ...
  test("postInstallVerify registers trusted hooks after installGrokPluginFromSource (native event map, T1)", async () => {
    ...
    const source = join(dirname(fileURLToPath(import.meta.url)), "..", "fixture")
