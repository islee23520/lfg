...
describe("doctor cli layout (#22)", () => {
  test("published-workspace when moduleUrl is under npm pack src/dist", async () => {
    ...
    try {
      await installGrokPluginFromSource({ home, sourceRoot: join(here, "..", "fixture") })
  ...
  test("cli fails when publish root bin.lfg is not bin/lfg.js shim (#22)", async () => {
