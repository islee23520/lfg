...
describe("docs/grok-adapter-core-port-strategy.md", () => {
  ...
  test("ADR revision 2026-07-17 lifts the TS-only lock for the lina-build fork (Rust-sidecar exception)", async () => {
    const text = await readFile(join(ROOT, "docs/grok-adapter-core-port-strategy.md"), "utf8")
    expect(text).toContain("## ADR Revision 2026-07-17 — Rust-sidecar exception for lina-build fork")
    expect(text).toContain("applies to the **lfg adapter** (`lfg-dev/src/`)")
    expect(text).toContain("fork-extension contract")
    expect(text).toContain("github.com/islee23520/lina-build")
    expect(text).toContain("Score C")
    expect(text).toContain("pi-rust vendor policy")
    expect(text).toContain("metaphorics/pi-rust")
    expect(text).toContain("lina-build/third_party/pi-rust/")
    expect(text).toContain("git submodule pinned at a recorded commit")
    expect(text).toContain("xai-grok-omo-senpi")
    expect(text).toContain("src/core/omo/ulw-loop/spawn-branches.ts")
    expect(text).toContain("Score A GrokBuild adapter scope stays 100/100")
    expect(text).toContain("Score B Full OMO host surface stays 89/100")
  })
})
