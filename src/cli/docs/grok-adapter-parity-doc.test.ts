...
describe("docs/grok-adapter-parity.md (plan task 1 + T5 contract)", () => {
  ...
  test("distinguishes core install parity from full OMO component parity for issue 36", async () => {
    ...
    expect(text).toContain("Current score after gap-fill train T0–T11")
    expect(text).toContain("**89/100**")
    expect(text).toContain("baseline was **88/100** after T2/T4/T5")
    expect(text).toContain("### Honest formula (this train)")
    ...
    expect(text).toContain("Not 90–93")
