...
describe("docs/grok-external-engine-orchestration.md", () => {
  test("documents the GPT-only app-server handoff contract", async () => {
    ...
    expect(text).toContain(".omo/external-engine/plan-ulw-plan-codex-skill-result.md")
    expect(text).toContain("--role coding --engine gpt")
    expect(text).toContain("$start-work")
    expect(text).toContain(".omo/external-engine/start-work-codex-skill-result.md")
    ...
    expect(text).toContain("skills/ulw-external-engine/SKILL.md")
