...
describe("docs/grok-adapter-agent-model-source-map.md", () => {
  ...
  test("maps every T5 upstream declaration file to a Grok target or no equivalent", async () => {
    ...
    expect(text).toContain("src/grok/agents/native-omo-agents.ts")
    expect(text).toContain("src/grok/agents/sync-lazycodex-agents-to-grok.ts")
    expect(text).toContain("src/grok/agents/lazycodex-agent-overrides.ts")
    expect(text).toContain("src/cli/setup/lfg-setup-tui-agents.ts")
    expect(text).toContain("prompt injection")
    expect(text).toContain(".omo/evidence/grokbuild-omo-porting/task-5-upstream-dts.txt")
    expect(text).toContain("lazycodex-worker-low")
    expect(text).toContain("medium")
    expect(text).toContain("high")
    expect(text).toContain("Disabled for Grok implementation")
    expect(text).toContain("lfg --json handoff plan --role coding --engine gpt")
    expect(text).toContain("difficulty-tier-routing.ts")
    expect(text).toContain("does not install an active LazyCodex coding model route")
  })
})
