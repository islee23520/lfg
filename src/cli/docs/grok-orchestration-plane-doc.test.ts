...
describe("docs/grok-orchestration-plane.md", () => {
  ...
  test("ADR revision 2026-07-17 brings omo-senpi into scope as Rust-sidecar plane (Plane C)", async () => {
    const text = await readFile(join(ROOT, "docs/grok-orchestration-plane.md"), "utf8")
    expect(text).toContain("## ADR Revision 2026-07-17 — omo-senpi as Rust-sidecar plane")
    expect(text).toContain("lfg adapter** (TypeScript, installed under `~/.grok/plugins/lfg`)")
    expect(text).toContain("Rust-sidecar (lina-build fork)")
    expect(text).toContain("Plane A")
    expect(text).toContain("Plane B")
    expect(text).toContain("Plane C")
    expect(text).toContain("Score C — Rust-sidecar scope")
    expect(text).toContain("xai-grok-omo-senpi")
    expect(text).toContain("src/core/omo/ulw-loop/spawn-branches.ts")
    expect(text).toContain("lina-build/crates/codegen/xai-grok-omo-senpi/src/spawn.rs")
    expect(text).toContain("spawn_subagent")
    expect(text).toContain("does NOT claim teammode full-team RPC parity")
  })
})
