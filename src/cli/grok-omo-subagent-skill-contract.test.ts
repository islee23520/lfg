...
describe("grok-omo-subagent skill contract", () => {
  for (const skillName of orchestrationSkills) {
    test(`${skillName} documents GrokBuild spawn_subagent mapping with explorer not explore`, async () => {
      ...
      expect(text).toContain('subagent_type: "explorer"')
      expect(text).toContain("coding_tool_adapter")
      expect(text).not.toMatch(/GrokBuild[\s\S]*subagent_type:\s*"explore"/)
    })
  }
})
