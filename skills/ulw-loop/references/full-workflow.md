...
if command -v omo >/dev/null 2>&1 && omo ulw-loop help >/dev/null 2>&1; then
  ...
  for candidate in "$HOME/.local/bin/omo" "$GROK_HOME/plugins/lfg/components/ulw-loop/dist/cli.js" "$GROK_HOME/plugins/lfg/hooks/lfg-native-ultrawork.js"; do
...
if [ -z "${ULW_LOOP_CLI:-}" ]; then
  ...
  printf '%s\n' "No ulw-loop-capable omo executable found; PATH omo may be the OpenCode CLI without the Codex ulw-loop subcommand, and cached ulw-loop CLI was not found under ${GROK_HOME:-$HOME/.grok}." >> "$NOTE" 2>/dev/null || true
  printf '%s\n' "Run lfg setup --run to refresh the lfg-owned GrokBuild plugin payload under ~/.grok/plugins/lfg." >&2
fi
```
...
```json
{
  "codeReview":{"by":"lfg-code-reviewer","recommendation":"APPROVE","codeQualityStatus":"CLEAR","reportPath":"grok-plugin-lfg/components/ulw-loop/test/fixtures/artifacts/code-review.md","evidence":"Diff review passed.","blockers":[]},
  "manualQa":{"by":"lfg-qa-executor","status":"passed","evidence":"CLI and data surfaces passed.","surfaceEvidence":[{"id":"surface-cli-pass","criterionRef":"C1","surface":"cli","invocation":"omo ulw-loop checkpoint --quality-gate-json sample-quality-gate.json --json","verdict":"passed","artifactRefs":["artifact-cli-pass"]},{"id":"surface-data-pass","criterionRef":"C2","surface":"data","invocation":"diff -u before-ledger.json after-ledger.json","verdict":"passed","artifactRefs":["artifact-data-diff"]}],"adversarialCases":[{"id":"adv-malformed-input","criterionRef":"C3","scenario":"malformed gate input omits manual QA evidence","expectedBehavior":"validator rejects ULW_LOOP_QUALITY_GATE_INVALID","verdict":"passed","artifactRefs":["artifact-cli-reject"]}],"artifactRefs":[{"id":"artifact-cli-pass","kind":"cli-transcript","description":"CLI pass artifact.","path":"grok-plugin-lfg/components/ulw-loop/test/fixtures/artifacts/cli-pass.txt"},{"id":"artifact-cli-reject","kind":"log","description":"Reject log artifact.","path":"grok-plugin-lfg/components/ulw-loop/test/fixtures/artifacts/rejection.txt"},{"id":"artifact-data-diff","kind":"data-diff","description":"Data diff artifact.","path":"grok-plugin-lfg/components/ulw-loop/test/fixtures/artifacts/data-diff.txt"}]},
  "gateReview":{"by":"lfg-gate-reviewer","recommendation":"APPROVE","reportPath":"grok-plugin-lfg/components/ulw-loop/test/fixtures/artifacts/gate-review.md","evidence":"Gate review passed.","blockers":[]},
  "iteration":{"fullRerun":true,"status":"passed","rerunCommands":["bunx vitest run grok-plugin-lfg/components/ulw-loop/test/quality-gate-doc.test.ts"],"evidence":"Focused rerun passed."},
