...
| `task(subagent_type="oracle", ...)` for final verification | `lfg --json handoff plan --role review --engine gpt --focus "..."`, then launch `handoff.launch.argv` exactly |
...
Put the full review brief, files, diffs, constraints, and required skills in `--focus`. Product review executes only through the external Codex handoff; do not spawn a LazyCodex-named agent.
