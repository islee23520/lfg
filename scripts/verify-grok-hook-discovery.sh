...
test -x "$GROK_BIN"
plugins/lfg/bin/grok-install-smoke.sh >/tmp/grok-hook-install-smoke.log
...
expected='/Users/ilseoblee/.grok/plugins/lfg/hooks/hooks.json'
...
for event, entries in data['hooks'].items():
    for entry in entries:
        for hook in entry['hooks']:
            assert hook['command'] == 'scripts/lfg-audit-hook.sh', (event, hook)
print('grok-hook-discovery=ok hooks=%d' % len(hooks))
PY
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
printf '{"hookEventName":"UserPromptSubmit","prompt":"xai-SECRET ghp_SECRET"}' \
  | GROK_PLUGIN_ROOT="/Users/ilseoblee/.grok/plugins/lfg" \
    GROK_PLUGIN_DATA="$TMP" \
    GROK_HOOK_EVENT="UserPromptSubmit" \
    /Users/ilseoblee/.grok/plugins/lfg/hooks/scripts/lfg-audit-hook.sh
