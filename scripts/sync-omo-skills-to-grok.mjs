...
function adaptUlwLoopReference(content) {
  return content
    ...
    .replaceAll("stable local installer bin or OMO-owned local CLI — same CLI, so PATH absence is not a blocker", "lfg packages a durable ulw-loop CLI as `lfg ulw-loop` / `lfg ulw` (upstream `omo` remains compatible if present)")
    ...
    .replaceAll("Install with npx lazycodex-ai install or set CODEX_LOCAL_BIN_DIR to a PATH directory.", "Use `lfg ulw-loop` (or `lfg ulw`). If PATH lacks lfg, run via the package bin after install (`npx @islee23520/lfg ulw-loop help`).")
    .replaceAll("Run lfg setup --run to refresh the lfg-owned GrokBuild plugin payload under ~/.grok/plugins/lfg.", "Use `lfg ulw-loop` (or `lfg ulw`). If PATH lacks lfg, run via the package bin after install (`npx @islee23520/lfg ulw-loop help`).")
