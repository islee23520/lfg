...
GROK_PLUGIN_DATA="$TMP" plugins/grok-harnessing/bin/lfg --json status >/tmp/lfg-state-status.json
...
assert schema['name'] == 'grok-build-state', schema
...
PY
GROK_PLUGIN_DATA="$TMP" plugins/grok-harnessing/bin/lfg --json doctor >/tmp/lfg-state-doctor.json
python3 - <<'PY'
import json
