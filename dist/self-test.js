...
// src/cli/self-test.ts
import { execFile } from "node:child_process";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";
var here = dirname(fileURLToPath(import.meta.url));
var LFG = join(here, "lfg.js");
var checks = [
...
async function commandOk(args, expected, env = {}) {
...
async function commandFails(args, expected) {
  const result = await execFileResult(process.execPath, [LFG, "--json", ...args]);
  return result.exitCode !== 0 && result.stdout.includes(expected);
}
function execFileResult(file, args, env = {}) {
