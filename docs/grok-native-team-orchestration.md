...
**Status:** Decision-Complete (2026-07-09)  
**Complements:** [`docs/grok-orchestration-plane.md`](grok-orchestration-plane.md), [`docs/grok-adapter-parity.md`](grok-adapter-parity.md), upstream [`packages/team-core`](https://github.com/code-yeongyu/oh-my-openagent/tree/main/packages/team-core) and `omo-senpi` team-service patterns, [`skills/teammode/SKILL.md`](../skills/teammode/SKILL.md)

This document records the **Grok-native mapping** for team orchestration. It fulfills **checkbox 4** of the `lfg-next-release-app-server-epic` (design only). The runtime MVP (todo 5) is explicitly **out of scope**; `teammode` inventory status **remains Deferred** and MUST NOT be flipped to `Grok-adapted`.
...
- durable state under `.omo/teams` using team-core concepts (from oh-my-openagent packages/team-core and omo-senpi team-service).
...
Failure always fail-closed: never report a "live" member without evidence in state. Skill detects capability (via existing model/runtime checks) and routes to fallback phrase above. This preserves honesty in `grok-adapter-parity.md` (teammode stays Deferred until full MVP verified).
...
- **Out of scope:** Full runtime implementation (todo 5), new MCP tools, flipping parity status, senpi app-server shim, tmux visualization.
