...
| `ulw-loop` | Grok-adapted | Grok-adapted with external CLI caveat | Keep project `.omo` awareness and upstream skill payload; durable CLI remains OMO-owned. | Prove pi-agent can use skill workflow and shared `.omo` context; do not claim packaged durable CLI. | `lfg-config-loader.mjs`; installed `ulw-loop` skill; docs state lfg does not package durable CLI. | pi-agent transcript showing skill/context usability. | Durable CLI is external; pi-agent skill surface unknown. | Should pi-agent proof require upstream `omo ulw-loop` CLI installed? |
