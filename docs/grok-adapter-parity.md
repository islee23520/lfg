...
| `ulw-loop` | `packages/omo-codex/MARKETPLACE.md`; `components/ulw-loop`; upstream OMO `skills/ulw-loop/` | `src/grok-adapter/assets/lfg-config-loader.mjs`; `src/grok-adapter/skills/ulw-loop/`; `project-omo-ledger.test.ts`; `omo-loader-runtime.integration.test.ts`; `run-grok-install.test.ts` | Project `.omo` awareness is installed fail-closed through `lfg-config-loader.mjs`; `ulw-loop` session count + active ledger presence is reported in hook context when `.omo/ulw-loop/<session>/ledger.jsonl` exists. The package installs the upstream OMO `ulw-loop` skill directory, including `references/full-workflow.md`; full durable CLI execution remains owned by OMO. | Grok-adapted |
| `ulw-plan` | upstream OMO `skills/ulw-plan/` | `src/grok-adapter/skills/ulw-plan/`; `run-grok-install.test.ts`; `post-install-verify.ts` | The package installs the upstream OMO `ulw-plan` skill directory, including `references/full-workflow.md`, intent references, and `scripts/scaffold-plan.mjs`; lfg does not replace it with a custom condensed workflow. | Grok-adapted |
| `start-work-continuation` | `packages/omo-codex/MARKETPLACE.md`; `components/start-work-continuation` | `src/grok-adapter/component-inventory.ts`; `grok-adapter-parity-doc.test.ts` | Boulder/start-work continuation is not yet driven as a Grok-native lifecycle workflow; the durable continuation CLI is not packaged, while Sisyphus Stop/SubagentStop hooks provide context only. | Deferred |
| `plan-mode-interception` | Grok runtime Plan Mode / `enter_plan_mode` expectation | `src/grok-adapter/assets/lfg-sisyphus-hooks.mjs`; `extension-hooks.test.ts`; `grok-adapter-parity-doc.test.ts` | Grok native Plan Mode interception is not implemented because no verified Grok hook/runtime surface is exposed for intercepting `enter_plan_mode`. Current `/ulw-plan` behavior is hook-time `additionalContext` guidance only, not native Plan Mode routing. | Deferred |
...
| `teammode` | `packages/omo-codex/plugin/components/teammode`; `packages/omo-codex/plugin/hooks/post-tool-use-checking-thread-title-hygiene.json` | `src/grok-adapter/component-inventory.ts`; `scripts/sync-omo-skills-to-grok.mjs`; `omo-skill-sync-script.test.ts` | The upstream 4.12.1 teammode skill payload is installed as an upstream-derived skill. The Codex `codex_app.create_thread` thread-title hook is not Grok-adapted yet. | Deferred |
| `lazycodex-executor-verify` | `packages/omo-codex/plugin/components/lazycodex-executor-verify`; `packages/omo-codex/plugin/hooks/subagent-stop-verifying-lazycodex-executor-evidence.json` | `src/grok-adapter/component-inventory.ts`; `grok-adapter-parity-doc.test.ts` | Upstream SubagentStop evidence verifier targets the Codex `lazycodex-executor` agent and writes `.omo/evidence` receipt enforcement. It needs Grok-specific agent naming/event adaptation before lfg can enable it. | Deferred |
