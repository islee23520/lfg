...
**Source of Truth**: [https://docs.x.ai/overview](https://docs.x.ai/overview), the official xAI developer documentation, the Grok Build skills/plugins/marketplaces guide, and the official SDK references.
...
- The Grok Spawn Adapter (`plugins/lfg/src/runtime-ts/index.ts`)
...
- SDK reference: [https://docs.x.ai/overview](https://docs.x.ai/overview)
- Voice, Image (Imagine), and Video APIs also exist but are secondary for agent orchestration work

## SDK Reference for Plugin Work

LFG plugin code should use official xAI SDK references for real Grok/xAI calls. The dependency-free smoke runtime must not hard-import SDK packages: `plugins/lfg/src/runtime-ts/index.ts`, `plugins/lfg/bin/lfg.ts`, `plugins/lfg/bin/lfg-mcp.ts`, hooks, and local smoke gates stay dependency-light. Any real SDK adapter belongs behind an optional module boundary under `plugins/lfg/src/` and must degrade to deterministic local envelopes when the SDK or credentials are absent.

This keeps two contracts separate:

- **Platform contract**: xAI docs + SDK references define how real Grok calls, Responses API usage, tool calling, and future stateful handoffs should be implemented.
...
- When adding real Grok API execution to a plugin module, cross-check official xAI SDK references and keep SDK imports optional.
...
- `plugins/lfg/src/runtime-ts/index.ts` (spawn adapter, `resolve_omo_model_profile`, Responses API fallback paths)
- `plugins/lfg/bin/lfg-mcp.ts` (MCP exposure of OMO tools)
